import random
populatin = [30, 50, 100]
rateCrossoverList = [0.9, 0.95, 1.0]
rateMutationList = [0.01, 0.1, 0.5]
epochsList = [100, 1000, 10000]
acumulador = 0
medias = []
cont = 0
for rateCrossover in rateCrossoverList:
    for p in populatin:
        acumulador = 0
        cont += 1
        for i in range(0, 30):
        ...
        if cont < 4:
            ...
            medias.append([mediaFinal,rateCrossover])

    if cont == 3:
        mediaFinal, x = min(medias, key=lambda x: x[0])
        populatin = []
        populatin.append(x)
        medias = []
        medias.append
        print("População com menor media: ", x, "Média: ", mediaFinal)
    cont += 1
if cont == 6:
    mediaFinal, rateCrossover = min(medias, key=lambda x: x[0])
    print("Taxa de crossover com menor media: ", rateCrossover, "Média: ", mediaFinal)
print(cont)
print(medias)
print(acumulador)
print(mediaFinal)